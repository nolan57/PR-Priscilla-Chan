        ...
        for lang, dict_path in dictionaries.items():
            with open(dict_path, 'r', encoding='utf8') as dict_file:
                for line in dict_file:
                    # Split by tab first, if no tab then split by space
...
def load_phoneme_dictionary() -> PhonemeDictionary:
    ...
    print(f"Loading phoneme dictionary from hparams:{config_dicts}")
    if config_dicts is not None:
        dicts = {}
        for lang, config_dict_path in config_dicts.items():
            print(f"Loading phoneme dictionary for language '{lang}' from {config_dict_path}")
            dict_path = pathlib.Path(hparams['work_dir']) / f'dictionary-{lang}.txt'
            if not dict_path.exists():
                print(f"Could not locate dictionary '{dict_path}', ")
                # 尝试使用配置中指定的路径，处理相对路径的情况
                dict_path = pathlib.Path(config_dict_path)
                # 如果是相对路径，尝试相对于当前工作目录和项目根目录的路径
                if not dict_path.is_absolute():
                    # 首先尝试相对于当前工作目录
                    if not dict_path.exists():
                        # 尝试相对于项目根目录 (DiffSinger/)
                        project_root = pathlib.Path(__file__).parent.parent
                        dict_path = project_root / config_dict_path
                        
                        # 再次检查是否存在
                        if not dict_path.exists():
                            # 最后尝试相对于项目的data目录
                            data_dir = project_root / 'data'
                            dict_path = data_dir / config_dict_path

                print(f"Using custom dictionary for language '{lang}': {dict_path}")
            if not dict_path.exists():
                raise FileNotFoundError(
                    f"Could not locate dictionary for language '{lang}'. Available path: {dict_path}"
                )
