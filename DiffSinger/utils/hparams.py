...
def set_hparams(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True):
    ...
    def load_config(config_fn):  # deep first
        print(f"[DEBUG] Loading config from {config_fn}")
        ...
        if 'base_config' in hparams_:
            ...
            for c in hparams_['base_config']:
                if c not in loaded_config:
                    print(f"[DEBUG] Loading config from c: {c}")
                    print(f"[DEBUG] Loaded config from loaded config: {loaded_config}")
                    if c.startswith('.'):
                        c = f'{os.path.dirname(config_fn)}/{c}'
                        c = os.path.normpath(c)
                    else:
                        # 处理非以.开头的相对路径，例如 configs/base.yaml
                        if not os.path.isabs(c):
                            # 尝试相对于项目根目录的路径（项目根目录是 utils 的父目录）
                            project_root = os.path.dirname(os.path.dirname(__file__))  # DiffSinger/
                            candidate_project_path = os.path.normpath(os.path.join(project_root, c))
                            
                            if os.path.exists(candidate_project_path):
                                c = candidate_project_path
                            else:
                                # 如果项目根目录下不存在，则尝试相对于当前配置文件的路径
                                config_dir = os.path.dirname(config_fn)
                                c = os.path.normpath(os.path.join(config_dir, c))
