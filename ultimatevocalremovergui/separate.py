...
# macOS: avoid abort when multiple OpenMP runtimes are loaded in different
# extension modules (common with brew/python/conda combinations). This is
# an unsafe workaround recommended by the OpenMP runtime when duplicate
# runtimes are detected; setting it early prevents import-time aborts.
# It must be set before importing libraries that may load OpenMP (numpy,
# torch, onnxruntime, etc.).
...
class SeperateAttributes:
    def __init__(self, model_data: ModelData, 
        ...
        self.primary_stem_native = model_data.primary_stem
        ...
        if model_data.process_method == MDX_ARCH_TYPE or model_data.process_method == 'mdx':
        ...
        if model_data.process_method == DEMUCS_ARCH_TYPE or model_data.process_method == 'demucs':
            ...
            self.demucs_source_map = getattr(model_data, 'demucs_source_map', None)
        ...
        if model_data.process_method in ['vr', VR_ARCH_TYPE]:
            self.check_label_secondary_stem_runs()
    ...
    def running_inference_console_write(self, is_no_write=False):
        self.write_to_console(DONE) if not is_no_write else None
    ...
    def write_audio(self, stem_path: str, stem_source, samplerate, stem_name=None):
        ...
        print(f'DEBUG: stem_path = {stem_path}')
        print(f'DEBUG: os.path.exists(os.path.dirname(stem_path)) = {os.path.exists(os.path.dirname(stem_path))}')
        
        def save_audio_file(path, source, samplerate):
            # Debug: Print save path
            print(f'DEBUG: Saving to path = {path}')
            print(f'DEBUG: source.shape = {source.shape}')
            print(f'DEBUG: wav_type_set = {self.wav_type_set}')
            print(f'DEBUG: samplerate = {samplerate}')
            source = spec_utils.normalize(source, self.is_normalization)
            # Ensure source is in (samples, channels) format for sf.write
            if source.shape[0] < source.shape[1]:
                source = source.T
                print(f'DEBUG: source.shape after transpose = {source.shape}')
            try:
                sf.write(path, source, samplerate, subtype=self.wav_type_set)
                print(f'DEBUG: sf.write succeeded')
                print(f'DEBUG: os.path.exists(path) = {os.path.exists(path)}')
                print(f'DEBUG: os.path.getsize(path) = {os.path.getsize(path) if os.path.exists(path) else 0}')
            except Exception as e:
                print(f'DEBUG: sf.write failed with error: {e}')
                import traceback
                traceback.print_exc()
        ...
        def save_with_message(stem_path, stem_name, stem_source):
            ...
            if is_deverb and is_not_ensemble:
                deverb_vocals(stem_path, stem_source, samplerate)
            
            save_audio_file(stem_path, stem_source, samplerate)
            self.write_to_console(DONE)
            
        def deverb_vocals(stem_path:str, stem_source, samplerate):
            ...
            save_audio_file(stem_path.replace(".wav", "_deverbed.wav"), stem_source_deverbed, samplerate)
            save_audio_file(stem_path.replace(".wav", "_reverb_only.wav"), stem_source_2, samplerate)
...
class SeperateMDXC(SeperateAttributes):        
    ...
    def seperate(self):
        ...
        if self.mdx_c_configs is None:
            raise ValueError("MDX-C configuration not loaded. Please ensure the model configuration file exists.")
        ...
        if (is_all_stems and is_not_ensemble_master and is_not_single_stem and is_not_secondary_model) or is_ensemble_4_stem and not self.is_pre_proc_model:
            ...
            if len(stem_list) == 1:
                ...
                # If using a multi-stem ensemble with exactly 2 stems, prefer the first stem
                if self.is_multi_stem_ensemble and len(stem_list) == 2:
                    source_primary = sources[stem_list[0]]
                else:
                    # Protect against cases where mdxnet_stem_select may be the sentinel ALL_STEMS
                    # `ALL_STEMS` is imported at module level via `from gui_data.constants import *`.
                    # Avoid re-importing here which would make ALL_STEMS a local variable
                    # and cause UnboundLocalError when referenced earlier in the function.
                    selected_stem = self.mdxnet_stem_select if self.mdxnet_stem_select != ALL_STEMS else stem_list[0]
                    source_primary = sources[selected_stem]
            if self.is_secondary_model_activated and self.secondary_model:
    ...
    def demix(self, mix):
        ...
        try:
            ...
            # If the model is wrapped by DataParallel, try to access its module attribute
            if hasattr(model, 'module'):
                try:
                    S = model.module.num_target_instruments
                except AttributeError:
                    # If module also doesn't have the attribute, determine target instruments count from config
                    if hasattr(model.module, 'config') and hasattr(model.module.config, 'training'):
                        S = len(model.module.config.training.instruments)
                    else:
                        # Default value, suitable for most cases
                        S = 2
            else:
                # If there's no module attribute, try other attribute names
                # Determine target instruments count from config
                if hasattr(model, 'config') and hasattr(model.config, 'training'):
                    S = len(model.config.training.instruments)
                else:
                    # Default value, suitable for most cases
                    S = 2
...
class SeperateDemucs(SeperateAttributes):
    def seperate(self):
        ...
        if is_no_cache:
            ...
            if hasattr(self, 'primary_model_name') and hasattr(self, 'primary_sources') and self.primary_model_name == self.model_basename and isinstance(self.primary_sources, np.ndarray) and self.pre_proc_model:
                source = self.primary_sources
            else:
...
class SeperateVR(SeperateAttributes):        
    ...
    def seperate(self):
        if hasattr(self, 'primary_model_name') and self.primary_model_name == self.model_basename and isinstance(self.primary_sources, tuple):
...
def main():
    """Main entry point for vocal separation from command line"""
    import argparse
    import os
    import sys
    from pathlib import Path
    ...
    parser.add_argument("--input_folder", type=str, required=True, help="Input folder containing audio files.")
    parser.add_argument("--output_folder", type=str, required=True, help="Output folder for separated vocals.")
    parser.add_argument("--model", type=str, required=True, help="Model name to use for separation.")
    parser.add_argument("--vocals_only", type=str, default="True", help="Only save vocal files.")
    ...
    parser.add_argument("--ignore_errors", action="store_true", help="Continue processing even if errors occur.")
    parser.add_argument("--debug", action="store_true", help="Enable debug output.")
    
    args = parser.parse_args()
    # Debug: show parsed args and the initial model lookup paths
    if args.debug:
        print(f"[DEBUG] UVR separate.py args: {args}")
    
    # Create output folder if it doesn't exist
    os.makedirs(args.output_folder, exist_ok=True)
    
    # Add the project root to sys.path so we can import UVR
    project_root = Path(__file__).parent
    sys.path.append(str(project_root))
    
    # Import required modules
    # Prefer the standalone ModelData module; fall back to UVR.ModelData if needed
    try:
        from modeldata import ModelData
    except Exception:
        try:
            ...
            print(f"Error importing required ModelData: {e}")
            return 1
    
    # Find the model file
    model_found = False
    model_paths = [
        # Check common locations and extensions for model files. Add MDX .ckpt support.
        project_root / "models" / "MDX_Net_Models" / f"{args.model}",
        # project_root / "models" / "MDX_Net_Models" / f"{args.model}.onnx
        # project_root / "models" / "MDX_Net_Models" / f"{args.model}.ckpt",
        # project_root / "models" / "MDX_Net_Models" / f"{args.model}.pth",
        project_root / "models" / "VR_Models" / f"{args.model}",
        # project_root / "models" / "VR_Models" / f"{args.model}.pth",
        # project_root / "models" / "Demucs_Models" / f"{args.model}.ckpt",
        # project_root / "models" / "Demucs_Models" / f"{args.model}.th",
        project_root / "models" / "Demucs_Models" / f"{args.model}",
        project_root / "models" / "Demucs_Models" / "v3_v4_repo" / f"{os.path.splitext(args.model)[0]}.yaml",
    ]

    if args.debug:
        print(f"[DEBUG] Checking model paths: {model_paths}")
    ...
    if not model_found:
        print(f"[ERROR] Model {args.model} not found in any of the expected locations.")
        return 1
    else:
        if args.debug:
    ...
    # Determine process method based on resolved model path (prefer folder over extension).
    model_path_str = str(model_path)
    lower_parent = str(model_path.parent).lower()

    if 'mdx_net_models' in lower_parent or model_path.suffix.lower() == '.onnx':
        process_method = MDX_ARCH_TYPE
    elif 'vr_models' in lower_parent or model_path.suffix.lower() in ('.pth', '.th', '.pt'):
        process_method = VR_ARCH_TYPE
    elif 'demucs_models' in lower_parent or model_path.suffix.lower() == '.yaml':
        process_method = DEMUCS_ARCH_TYPE
    else:
        # Fallback to extension-based detection if parent folder is ambiguous
        if args.model.endswith('.onnx'):
            process_method = MDX_ARCH_TYPE
        elif args.model.endswith(('.pth', '.th', '.pt')):
            process_method = VR_ARCH_TYPE
        elif args.model.endswith(('.ckpt', '.yaml')):
            # Prefer MDX for .ckpt when not under Demucs folder, since many MDX models use .ckpt
            process_method = MDX_ARCH_TYPE
        else:
            print(f"Unsupported model type for model: {args.model}")
            return 1
    
    # Create model data
    model_data = ModelData(
        model_name=args.model,
        selected_process_method=process_method,
        is_secondary_model=False
    )
    # Ensure the resolved model path and basename match the path we found above
    ...
    # Set is_mdx_c based on model name
    if "MDX23C" in args.model.upper() or "MDX_C" in args.model.upper():
        model_data.is_mdx_c = True
    else:
        model_data.is_mdx_c = False
    # If this is an MDX-C model, attempt to load its YAML config from the
    # bundled `model_data/mdx_c_configs` directory so the separation code
    # has a valid `mdx_c_configs` object.
    if model_data.is_mdx_c:
        try:
            cfg_dir = project_root / "models" / "MDX_Net_Models" / "model_data" / "mdx_c_configs"
            if cfg_dir.exists():
                candidates = list(cfg_dir.glob("*.yaml"))
                chosen = None
                basename = os.path.splitext(args.model)[0].lower()
                # Try to find a config whose name matches the model basename
                for c in candidates:
                    name = c.stem.lower()
                    if name in basename or basename in name:
                        chosen = c
                        break
                # Prefer an 8k config when model name contains '8k'
                if not chosen and '8k' in basename:
                    for c in candidates:
                        if '8k' in c.stem.lower():
                            chosen = c
                            break
                ...
                if chosen:
                    ...
                    # Convert nested dict -> ConfigDict (simple attr access)
                    def dict_to_config(d):
                        ...
                        if isinstance(d, dict):
                            ...
                            return config
                        return d
                    model_data.mdx_c_configs = dict_to_config(cfg_dict)
                    if args.debug:
                        print(f"[DEBUG] Loaded MDX-C config: {chosen}")
        except Exception as e:
            if args.debug:
                print(f"[DEBUG] Failed loading mdx_c config: {e}")
    if args.debug:
        print(f"[DEBUG] Created ModelData: model_name={model_data.model_name}, process_method={process_method}, model_path={model_path}")
    
    # Override some model settings based on command line args
    model_data.is_gpu_conversion = -1 if args.no_cuda else 0  # -1 for CPU, 0+ for GPU
    model_data.is_deverb_vocals = False
    
    # Set primary and secondary stems on model_data
    if args.vocals_only.lower() == "true":
        model_data.is_primary_stem_only = True
        model_data.is_secondary_stem_only = False
        model_data.primary_model_primary_stem = VOCAL_STEM
        model_data.secondary_stem = INST_STEM
    else:
        # Default to vocals and instrumental if not specifically set
        model_data.primary_model_primary_stem = VOCAL_STEM 
        model_data.secondary_stem = INST_STEM 
    
    # Set vocal only if requested
    if args.vocals_only.lower() == "true":
        model_data.is_primary_stem_only = True
        model_data.is_secondary_stem_only = False
    # Set vocal only if requested
    ...
    for i, audio_file in enumerate(audio_files, 1):
        try:
            ...
            def _console_writer(*args, base_text=None, **kwargs):
                """Console writer wrapper that accepts an optional `base_text` kwarg.

                Many internal calls pass `base_text=''` which the builtin
                `print` does not accept. This wrapper supports that argument and
                forwards other values to `print`.
                """
                try:
                    if base_text is None or base_text == '':
                        ...
                        if args:
                            first, *rest = args
                            print(f"{base_text}{first}", *rest, **kwargs)
                        else:
                            print(base_text, **kwargs)
                except TypeError:
                    # Fallback: just try printing the args
                    print(*args)

            process_data = {
                'audio_file': str(audio_file),
                'audio_file_base': audio_file.stem,
                'export_path': str(output_folder),
                'set_progress_bar': lambda a, b=0: None,  # No progress bar in command line
                'write_to_console': _console_writer,
                'cached_source_callback': lambda a, model_name: (None, None),
                'cached_model_source_holder': lambda a, b, c: None,
            ...
            if model_data.process_method == MDX_ARCH_TYPE:
            ...
            elif model_data.process_method == VR_ARCH_TYPE:
                ...
                print(f"Unsupported process method: {model_data.process_method}")
    ...
    print(f"\nProcessing completed. Success: {success_count}, Errors: {error_count}")
