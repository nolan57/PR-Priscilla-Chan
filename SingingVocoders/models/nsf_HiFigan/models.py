...
# Custom weight_norm that handles MPS devices
def conditional_weight_norm(module):
    """Apply weight norm conditionally based on device type."""
    if hasattr(torch.backends, 'mps') and torch.backends.mps.is_available() and torch.backends.mps.is_built():
        # For MPS devices, we can't use weight_norm directly
        # Instead, we'll just return the original module
        # Alternative: implement a manual weight normalization
        return module
    else:
...
class ResBlock1(torch.nn.Module):
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        self.convs1 = nn.ModuleList([
            conditional_weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=dilation[0],
                               padding=get_padding(kernel_size, dilation[0]))),
            conditional_weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=dilation[1],
                               padding=get_padding(kernel_size, dilation[1]))),
            conditional_weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=dilation[2],
                               padding=get_padding(kernel_size, dilation[2])))
        ])
        self.convs1.apply(init_weights)

        self.convs2 = nn.ModuleList([
            conditional_weight_norm(Conv1d(channels, channels, kernel_size, 1, dilation=1,
    ...
    def remove_weight_norm(self):
        for l in self.convs1:
            try:
                ...
                # Weight norm was not applied, skip removal
                pass
...
class Generator(torch.nn.Module):
    def __init__(self, h):
        ...
        self.conv_pre = conditional_weight_norm(Conv1d(h.num_mels, h.upsample_initial_channel, 7, 1, padding=3))   
        ...
        for i, (u, k) in enumerate(zip(h.upsample_rates, h.upsample_kernel_sizes)):
            ch //= 2
            self.ups.append(conditional_weight_norm(ConvTranspose1d(2 * ch, ch, k, u, padding=(k - u) // 2)))
        ...
        self.conv_post = conditional_weight_norm(Conv1d(ch, 1, 7, 1, padding=3))
...
class DiscriminatorP(torch.nn.Module):
    def __init__(self, period, kernel_size=5, stride=3, use_spectral_norm=False):
        ...
        self.period = period
        # Use conditional weight norm instead of always applying weight_norm
        def norm_f(module):
            if use_spectral_norm:
                return spectral_norm(module)
            else:
                return conditional_weight_norm(module)
